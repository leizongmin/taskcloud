 ...
 */
session.create = function (id, access_token) {
	var sandbox = {
		require:	session_require,						// require函数
		rest:		rest,									// REST模块
		Jscex:		Jscex,									// Jscex异步编程模块
		task:		new TaskSession(id, access_token),		// 用于操作任务队列
		util: {												// 工具集
...
//---------------------------------------------
/** 操作任务队列 */
var TaskSession = function (id, access_token) {
	this.id = id;
...
TaskSession.prototype.start = function () {
	return taskqueue.running(this.id);
}
/* 任务结束 */
TaskSession.prototype.end = function () {
	return taskqueue.running(this.id, false);
}
/* 删除自身 */
TaskSession.prototype.remove = function () {
	return taskqueue.remove(this.id);
}
/* 取任务信息 */
TaskSession.prototype.info = function () {
	return taskqueue.get(this.id);
}
/* 重新设置任务队列 */
TaskSession.prototype.modify = function (isAuto, t) {
	return taskqueue.add(this.id, isAuto, t.template, this.access_token, t.cycle, t.auto_start, t.auto_end);
}
/* 修改运行周期 */
