/**
 * taskcloud虚拟机
 *
 * @author leizongmin<leizongmin@gmail.com>
 * @version 0.1
 */
 
var templatecache = require('./templatecache');		// 模版缓存模块
var taskqueue = require('./taskqueue');				// 任务队列模块
var datacache = require('./datacache');				// 数据缓存模块
var session = require('./session');					// 任务会话模块
var rest = require('./rest');						// REST网络访问模块
var cmdparser = require('./cmdparser');				// 命令/数据解析模块

var taskvm = module.exports;

taskvm.logger = function () { console.log.apply(null, arguments); }
var debug = function () {
	if (taskvm.logger)
		taskvm.logger.apply(null, arguments);
}

/** 默认任务队列扫描周期 */
taskvm.cycle = 1000;	// ms
var _taskvm_auto_scan = false;

/**
 * 初始化
 *
 * @param {string} template_dir 任务模版所在目录
 * @param {int} queue_cycle 扫描周期(ms)
 */
taskvm.init = function (template_dir, queue_cycle) {
	// 配置各个模块的logger
	templatecache.logger = taskvm.logger;
	taskqueue.logger = taskvm.logger;
	datacache.logger = taskvm.logger;
	session.logger = taskvm.logger;
	rest.logger = taskvm.logger;
	cmdparser.logger = taskvm.logger;
	
	// 配置任务模版所在目录
	if (template_dir)
		templatecache.dirname = template_dir;
		
	// 配置队列扫描周期，必须大于100ms
	if (queue_cycle >= 100) 
		taskvm.cycle = queue_cycle;
		
	// 输出配置信息
	debug('Init taskvm: cycle=' + taskvm.cycle + 'ms,  ' + 'template_dir=' + template_dir);
}

/**
 * 加入任务
 *
 * @param {string} code 命令代码
 * @param {string} template_path 模版目录
 * @return {bool}
 */
taskvm.exec = function (code, template_path) {
	var cmd = cmdparser.parser(code);
	if (!cmd) {
		debug('Parsing task start-code error: bad format.');
		return false;
	}
	
	// 任务默认参数
	var t = cmd.task;
	if (typeof t.id == 'undefined' || typeof t.template == 'undefined') {
		debug('Parsing task start-code error: missing id or template.');
		return false;
	}
	if (isNaN(t.id)) {
		debug('Parsing task start-code error: id must a number.');
		return false;
	}
	t.template = t.template.trim();
	if (t.template == '') {
		debug('Parsing task start-code error: template cannot be empty.');
		return false;
	}
	if (template_path) {
		t.template = template_path + '/' + t.template;
	}
	if (typeof t.auto == 'undefined')
		t.auto = true;
	t.auto = t.auto ? true : false;
	if (typeof t.cycle == 'undefined')
		t.cycle = 0;
	if (typeof t.access_token == 'undefined')
		t.access_token = '';
	if (typeof t.start == 'undefined')
		t.start = 0;
	if (typeof t.end == 'undefined')
		t.end = 0;
	if (isNaN(t.start))
		t.start = 0;
	if (isNaN(t.end))
		t.end = 0;
	if (isNaN(t.cycle))
		t.cycle = 0;
	if (t.cycle < 1)
		t.auto = false;
	
	// 载入任务模版
	var s = templatecache.get(t.template);
	if (!s) {
		debug('Loading task template [' + t.template + '] error!');
		return false;
	}
	
	// 加入任务到队列
	taskqueue.add(t.id, t.auto, t.template, t.access_token, t.cycle, t.start, t.end);
	
	// 加入预设数据
	datacache.set(t.id + '/arguments', cmd.data);
	
	return true;
}

/**
 * 开始自动扫描队列，并运行任务
 */
taskvm.start = function () {
	_taskvm_auto_scan = true;
	if (taskvm.cycle >= 100)  {
		auto_scan_and_run();
		debug('Start taskvm.');
	}
	else
		debug('Start taskvm fail: cycle=' + taskvm.cycle + 'ms');
}
var auto_scan_and_run = function () {
	var list = taskqueue.scan();
	for (i in list) {
		var t = list[i];
		run_in_sandbox(t);
	}
	
	// 继续自动扫描
	if (_taskvm_auto_scan)
		setTimeout(auto_scan_and_run, taskvm.cycle);
}
var run_in_sandbox = function (t) {
	try {
		var s = templatecache.get(t.template);
		if (!s) {
			debug('Start task [' + t.task_id + '] fail: missing template. [' + t.template + ']');
			return false;
		}
		// 创建环境
		var sandbox = session.create(t.task_id, t.access_token);
		s.runInNewContext(sandbox);
		return true;
	}
	catch (err) {
		debug('Start task [' + t.task_id + '] fail: ' + err);
		return false;
	}
}


/**
 * 停止自动扫描队列
 */
taskvm.stop = function () {
	_taskvm_auto_scan = false;
	debug('Stop taskvm.');
}

/**
 * 获取任务队列
 *
 * @return {object} 
 */
taskvm.list = function () {
	return taskqueue.queue;
}

/**
 * 删除指定任务
 *
 * @param {int} id 任务ID
 */
taskvm.kill = function (id) {
	return taskqueue.remove(id);
}

/**
 * 运行一次指定任务
 *
 * @param {int} id 任务ID
 * @return {bool}
 */
taskvm.once = function (id) {
	var t = taskqueue.get(id);
	if (!t)
		return false;
	
	return run_in_sandbox(t);
}